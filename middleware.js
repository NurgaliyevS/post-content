...
export async function middleware(request) {
  ...
  if (request.nextUrl.pathname === '/api/create-checkout') {
    const ip = request.ip || request.headers.get('x-real-ip') || request.headers.get('x-forwarded-for') || 'anonymous';
    const now = Date.now();
      ...
      const response = new NextResponse(
        ...
        }),
        {
          ...
          headers: {
            ...
            'X-RateLimit-Reset': (Math.ceil(ipData.timestamp / 1000) + timeWindow).toString()
          }
        }
      );
