...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(
      `Found ${publishedPosts.length} published posts to update metrics for`
    );
    ...
    for (const post of publishedPosts) {
      try {
        ...
        const redditResponse = await fetch(
          `https://oauth.reddit.com/by_id/t3_1jsxv5h?sr_detail=true`,
          {
            headers: {
        ...
        const analyticsResponse = await fetch(
          `https://oauth.reddit.com/api/info?id=t3_1jsxv5h&rawjson=1`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "User-Agent": "RedditScheduler/1.0.0",
        ...
        const postAboutResponse = await fetch(
          `https://oauth.reddit.com${postData.permalink}/about`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "User-Agent": "RedditScheduler/1.0.0",
        ...
        const aboutData = await postAboutResponse.json();
        ...
        const impressions =
          ...
          postData?.all_awardings?.reduce(
            (total, award) => total + award.count,
            0
          ) ||
          0;
        ...
        console.error(
          `Error updating metrics for post ${post.redditPostId}:`,
          error
        );
        results.push({
          postId: post.redditPostId,
